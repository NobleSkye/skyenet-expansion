...
export class GameMenu {
  ...
  constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, atlasManager: AtlasManager) {
    this.canvas = canvas;
    this.ctx = ctx;
  ...
  private handleMainMenuClick(clickX: number, clickY: number) {
    ...
    if (clickXBase >= shipArea.x && clickXBase <= shipArea.x + shipArea.width &&
        clickYBase >= shipArea.y && clickYBase <= shipArea.y + shipArea.height) {
    ...
    if (clickXBase >= startButton.x && clickXBase <= startButton.x + startButton.width &&
        clickYBase >= startButton.y && clickYBase <= startButton.y + startButton.height) {
  ...
  private handleShipSelectionClick(clickX: number, clickY: number) {
    ...
    if (clickXBase >= backButton.x && clickXBase <= backButton.x + backButton.width &&
        clickYBase >= backButton.y && clickYBase <= backButton.y + backButton.height) {
      this.state.currentScreen = 'main';
    }
  }

  public render() {
    console.log("Menu render called, current screen:", this.state.currentScreen);
  ...
  private renderMainMenu() {
    console.log("Rendering main menu, canvas size:", this.canvas.width, "x", this.canvas.height);
    ...
    this.ctx.fillText(this.state.selectedShip.name, baseWidth * 0.1, baseHeight * 0.35);
    this.ctx.fillText(this.state.selectedShip.description, baseWidth * 0.1, baseHeight * 0.4);
    ...
    if (this.atlasManager.areAllLoaded()) {
      ...
      const shipX = Math.max(baseWidth * 0.6, Math.min(baseWidth * 0.9, mouseXBase));
      const shipY = Math.max(baseHeight * 0.3, Math.min(baseHeight * 0.7, mouseYBase));
      ...
      this.ctx.textAlign = 'center';
      this.ctx.fillText('Click on ship to change', baseWidth * 0.75, baseHeight * 0.8);
    } else {
