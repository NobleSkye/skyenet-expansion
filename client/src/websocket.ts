import { AuthenticationMessage, AuthenticationMessageCallback, StatusMessage } from "../../core/src/schemas";
    ...
    socket.onmessage = (message) => {
        ...
        switch(json.type) {
            ...
            case WebSocketMessageType.AuthenticationCallback:
                ...
                if(!result.success) break;
                handleAuthenticationCallbackMessage(json, socket);
                break;
            default: break;
        }
    };
...
function handleStatusMessage(msg: MessageType.StatusMessage, socket: WebSocket) {
    if(msg.status === "connected") {
        socket.send(JSON.stringify(AuthenticationMessage.parse({})));
    }
}

function handleAuthenticationCallbackMessage(msg: MessageType.AuthenticationMessageCallback, socket: WebSocket) {
